...
function mayuscula(nombre) {
   ...
   let mayus = nombreArray[0].toUpperCase();
   nombreArray.shift();
   nombreArray.unshift(mayus);
   nombre = nombreArray.join('');
   return nombre;
}
...
function filter(arrayOfStrings) {
   ...
   arrayOfStrings.forEach( (elemento) => { 
      var palabra = elemento.split('');
      if (palabra[0].includes('a') || palabra[0].includes('A')){;
